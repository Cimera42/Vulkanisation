...
bool createCommandBuffers()
{
    ...
    for(int i = 0; i < commandBuffers.size(); i++)
    {
        ...
        vkCmdBeginRenderPass(commandBuffers[i], &renderPassBeginInfo, VK_SUBPASS_CONTENTS_INLINE );
            ...
            vkCmdBindPipeline(commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, simplepipeline);

            for(int j = 0; j < meshes.size(); j++)
            {
                vkCmdBindDescriptorSets(commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &descriptorSets[j], 0, NULL);
                vkCmdBindVertexBuffers(commandBuffers[i], 0, 1, &meshes[j].vertexBuffer.buffer, &offsets);
                vkCmdBindIndexBuffer(commandBuffers[i], meshes[j].indexBuffer.buffer, 0, VK_INDEX_TYPE_UINT32);
                vkCmdDrawIndexed(commandBuffers[i], meshes[j].indices.size(), 1,0,0,1);
            }
...
bool doDepthImage()
{
    ...
    depthImageCreateInfo.format = VK_FORMAT_D16_UNORM;                          // notice me senpai!
...
bool loadShaders()
{
    ...
    std::vector<VkVertexInputAttributeDescription> vertexAttributeDescriptionSimple(4);
    ...
    std::vector<VkVertexInputAttributeDescription> vertexAttributeDescriptionNormal(2);
...
bool createPipeline()
{
    ...
    if(result != VK_SUCCESS)
    {
        std::cout << "Pipeline creation failed (" << result << ")" << std::endl;
        return false;
    }
...
bool loadFramebuffer()
{
    ...
    VkFormat fbColourFormat = VK_FORMAT_R8G8B8A8_UNORM;
...
int main()
{
    ...
    VkSemaphore imageAvailableSemaphore, renderingCompleteSemaphore;
    ...
    while (!glfwWindowShouldClose(window) && glfwGetKey(window, GLFW_KEY_ESCAPE) != GLFW_PRESS)
    {
        ...
        //else
        //    std::cout << "Draw queue submitted" << std::endl;
